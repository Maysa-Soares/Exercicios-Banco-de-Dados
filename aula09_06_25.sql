...
--estudos view
-- view
-- Seleção simples da tabela PRODUTOS
SELECT * FROM products;

-- Seleciono as colunas especificas
SELECT
	product_id,
	product_name,
	unit_price
...
-- Consultando a tabela virtual (VIEW)
SELECT * FROM vw_products;

-- E se eu precisar mudar a tabela?
CREATE OR REPLACE VIEW vw_products AS
SELECT
	product_id,
	product_name,
	unit_price,
	units_in_stock -- Add coluna
FROM products;
-- Consultando a tabela virtual (VIEW)
SELECT * FROM vw_products;

CREATE OR REPLACE VIEW vw_func_agregacao AS 
SELECT
	product_id,
	product_name,
	category_id AS "ID_Categoria",
	AVG(unit_price) AS "Valor Médio",
	MIN(unit_price) AS "Menor Valor",
	MAX(unit_price) AS "Maior Valor"
FROM products
GROUP BY product_id
ORDER BY category_id ASC
 

SELECT * FROM vw_func_agregacao;
...
SELECT
    ...
    o.required_date AS data_requerida,
    o.shipped_date AS data_envio,
    
    c.company_name AS nome_cliente,
    c.contact_name AS nome_contato,
    ...
    (od.unit_price * od.quantity * (1 - od.discount)) AS total_item,
...
SELECT * FROM vw_relatorio_pedidos;

-- aula view continuação

--excluir view
drop view vw_produtos;

--tarefa dois
CREATE VIEW vw_func_agrega AS
SELECT 
    c.Category_name AS categoria,
    COUNT(p.Product_id) AS total_produtos,
    AVG(p.Unit_price) AS preco_medio,
    MAX(p.Unit_price) AS preco_maximo,
    MIN(p.Unit_price) AS preco_minimo
FROM 
    Products p
JOIN 
...
select * from vw_func_agrega;

--tarefa três 
CREATE VIEW vw_round_trunc AS
SELECT 
    Product_id,
    Product_name,
    Unit_price AS preco_original,
    ROUND(CAST(Unit_price AS numeric), 2) AS preco_arredondado,
    TRUNC(CAST(Unit_price AS numeric), 2) AS preco_truncado
FROM 
    Products;

select * from vw_round_trunc; 
...
-- jeito mais coreto que eu achei
SELECT
    first_name,
    UPPER(first_name) AS letrasMAIUS,
    LOWER(first_name) AS letrasMinusc,
    LENGTH(first_name) AS sizesString,
    INITCAP(first_name) AS nomeFormatado
FROM employees;

--tarefa cinco
CREATE OR REPLACE VIEW vw_names_corrigidos AS
SELECT
    employee_id,
    INITCAP(LOWER(first_name)) AS first_name_formatado,
    INITCAP(LOWER(last_name)) AS last_name_formatado,
    LENGTH(first_name || ' ' || last_name) AS full_name_length
FROM employees;

select * from vw_names_corrigidos;


-- dia 09/06/25

--view consulta
CREATE OR REPLACE VIEW vw_namess AS
SELECT
    customer_id,
    company_name,
    contact_name,
    contact_title,
    REPLACE(contact_title, 'Owner', 'Proprietario') AS contact_title_corrigido
FROM customers;

-- trocar Owner para Proprietario 
SELECT
	contact_name,
	contact_title,
	REPLACE(contact_title, 'Owner', 'Proprietario')
FROM vw_namess

--REPLACE (Tabelas customers)
SELECT
	contact_name AS Nome,
--contact_title
	Replace(contact_title, 'Owner', 'Proprietario') AS "Cargo"
FROM customers;

--substring (sem tabela)
SELECT
	'ABC-1234',
	SUBSTRING('ABC-1234', 1, 3) AS "nome_recortado",
	SUBSTRING('ABC-1234', 5, 4) AS "num_recortado";

--STRPOS (sem tabela)
SELECT
	'ABC-1234',
	'Rillary Sousa',
	STRPOS('ABC-1234', '-') AS "tam_recortado";

--strpos + substring (sem tabela)
SELECT
	'ABC-1234',
	SUBSTRING('ABC-1234', STRPOS('ABC-1234', 'A'), 3) AS "letras_recortado1",
	SUBSTRING('ABC-1234', STRPOS('ABC-1234', 'A'), STRPOS('ABC-1234', 'C')) AS "letras_recortado_b",
	SUBSTRING('ABC-1234', 1, STRPOS('ABC-1234', '-') -1) AS "letras_recortado_t",
	SUBSTRING('ABC-1234', 1,3) AS "letras_recortado";
