...
--querys de agregação e agrupamento 

--ex 1: RETORNA total de linhas
SELECT COUNT(contact_name) "TotaldeLinhas_Custormers"
FROM customers; 

--ex 2: RETORNA total de linha(COUNT não considera valores nulos)
SELECT COUNT(region)
FROM customers;
...
--(SUM) faz soma dos valores de uma coluna

--ex 1: retorna total de uniddes do produto x em estoque
SELECT SUM (units_in_stock) AS "TotalemEstoque"
FROM products;

--ex 1: retorna valor total de  estoque
SELECT SUM (unit_price) AS "ValorTotalEstoque"
FROM products;
...
--(AVG, MIN, MAX) retorna média, menor e maior valor da coluna
SELECT 
	AVG (unit_price) AS mediaPreco,
	MIN (unit_price) AS menorPreco,
	MAX (unit_price) AS maiorPreco
FROM products;


--(GROUP BY) agrupa valor de acordo com a coluna
-- GROUP BY * SUM
SELECT
	supplier_id AS fornecedor , 
	SUM (units_in_stock) AS "Total em Estoque"
FROM products
GROUP BY supplier_id;

--segunda - feira 14/04/25
 -----------------------------------------------------------------------------------------------------------
--(GROUP BY) agrupa valor de acordo com uma coluna
-- GROUP BY * ORDER BY ordenar o agrupamento 
SELECT 
	country AS paises,
	COUNT(*) AS quantidade
FROM 
...
--(GROUP BY) agrupa valor de acordo com uma coluna
-- GROUP BY * WHERE ou HAVING
SELECT 
	country AS paises,
	COUNT(*) AS quantidade
FROM 
	customers
WHERE contact_title = 'Owner' --filtrando antes de agrupar
GROUP BY country;
